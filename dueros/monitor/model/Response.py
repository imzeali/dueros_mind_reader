...
class Response:
    ...
    def get_output_speech(self):
        if Utils.check_keys_in_dict(self.data, ['response', 'outputSpeech']):
            return self.data['response']['outputSpeech']

    def get_should_end_session(self):
        if Utils.check_keys_in_dict(self.data, ['response', 'shouldEndSession']):
    ...
    def get_slot_name(self):
        ...
        if Utils.check_keys_in_dict(self.data, ['response', 'directives']):
            ...
            if directive and Utils.check_keys_in_dict(directive[0], ['slotToElicit']):
                return directive[0]['slotToElicit']
        return None

    def get_reprompt(self):

        if Utils.check_keys_in_dict(self.data, ['response', 'reprompt', 'outputSpeech']):
